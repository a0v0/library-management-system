...
?>
    ...
    <body>
        ...
        <div class="w-full flex flex-col justify-center items-center pl-3 pt-30 text-center">
            <div>
                <h3 class="text-4xl font-semibold text-slate-800">Manage Authors</h3>
                <p class="text-slate-500">View and manage all authors in the system.</p>
            </div>
        </div>
        ...
        <div class="relative flex flex-col w-11/12 max-w-5xl mx-auto my-10 p-6 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-gray-700 shadow-lg rounded-lg">
            ...
            <div class="mx-auto my-6 w-full">
                <table id="authorsTable" class="w-full text-left table-auto min-w-max border-collapse border border-slate-200 bg-white rounded-lg shadow-md">
                    <thead>
                        <tr>
                            ...
                            <th class="p-4 border border-slate-200 bg-slate-50">
                                <p class="text-sm font-normal leading-none text-slate-500">Creation Date</p>
                            </th>
                            <th class="p-4 border border-slate-200 bg-slate-50">
                                <p class="text-sm font-normal leading-none text-slate-500">Updation Date</p>
                            </th>
                            <th class="p-4 border border-slate-200 bg-slate-50">
                        ...
                        if ($query->rowCount() > 0) {
                            foreach ($results as $result) { ?>
                                <tr class="hover:bg-slate-50 border-b border-slate-200">
                                    ...
                                    <td class="p-4 py-5 border border-slate-200"><?php echo htmlentities(date("F j, Y, g:i a", strtotime($result->creationDate))); ?></td>
                                    <td class="p-4 py-5 border border-slate-200"><?php echo htmlentities(date("F j, Y, g:i a", strtotime($result->UpdationDate))); ?></td>
                                    <td class="p-4 py-5 border border-slate-200">
                                        <a href="edit-author.php?athrid=<?php echo htmlentities($result->id); ?>" class="text-blue-500 hover:underline">Edit</a>
                                        <a href="manage-authors.php?del=<?php echo htmlentities($result->id); ?>" onclick="return confirm('Are you sure you want to delete?');" class="text-red-500 hover:underline ml-2">Delete</a>
                                    </td>
                                </tr>
                        <?php $cnt++;
                            }
        ...
        <script>
            function filterTable() {
                ...
                const tableRows = document.querySelectorAll("#authorsTable tbody tr");
            ...
            document.getElementById("nextPage").addEventListener("click", () => {
                const totalRows = document.querySelectorAll("#authorsTable tbody tr").length;
                const totalPages = Math.ceil(totalRows / rowsPerPage);
